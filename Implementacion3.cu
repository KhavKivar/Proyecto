...
int main(int argc, char *argv[])
{
  ...
  printf("Tiempo Implementacion 2 (GPU): %f ms\n", milliseconds);
  ...
  conv_img_gpu_f3<<<grid_size, 1, kernel_size * kernel_size * sizeof(float)>>>(d_img, d_kernel, d_imgf, Nx, Ny, kernel_size);    
  ...
  printf("Tiempo  Implementacion 1 (GPU): %f ms\n", milliseconds);
